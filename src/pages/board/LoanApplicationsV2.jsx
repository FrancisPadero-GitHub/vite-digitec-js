...
function LoanApplicationsV2() {
  ...
  // mutation hooks
  const { mutate: mutateUpdateLoanApp, isPending: isUpdateLoanAppPending } =
    useEditLoanApp();
  const { mutate: mutateAddLoanAcc, isPending: isAddLoanAccPending } =
    useAddLoanAcc();
  ...
  const principalDecimal = watchLoanAcc("principal")
    ? new Decimal(watchLoanAcc("principal"))
    : new Decimal(0);
  ...
  const selectedProduct =
    loanProducts?.find((product) => product.product_id === loan_product_id) ||
    null;
            ...
            return (
              ...
              >
                ...
                <td>
                  {loanProduct ? (
                    <span
                      className={`font-semibold ${LOAN_PRODUCT_COLORS[loanProduct]}`}
                    >
                      {loanProduct}
                    </span>
                  ) : (
                ...
                <td>
                  {appStatus ? (
                    <span
                      className={`badge font-semibold ${LOAN_APPLICATION_STATUS_COLORS[appStatus]}`}
                    >
                      {appStatus}
                    </span>
                  ) : (
          ...
          <div className="pr-2">
            ...
            <div className="p-3 bg-blue-50 rounded-lg border-2 border-blue-200 mb-4">
                ...
                !isLoanAlreadyApproved && (
                  <div className="p-3 bg-green-50 border border-green-300 rounded-lg flex items-start gap-2">
                    ...
                    <p className="text-sm text-green-800">
                      <strong>Approved:</strong> Click &ldquo;Next&rdquo; to
                      review and confirm loan release details.
                    </p>
                  </div>
                )}
              ...
              {watchLoanApp("status") === "Denied" && (
                <div className="p-3 bg-red-50 border border-red-300 rounded-lg flex items-start gap-2">
                  <p className="text-sm text-red-800">
                    <strong>Denied:</strong> This application will be declined.
              ...
              {watchLoanApp("status") === "Cancelled" && (
                <div className="p-3 bg-red-50 border border-red-300 rounded-lg flex items-start gap-2">
                  <p className="text-sm text-red-800">
                    <strong>Cancelled:</strong> This application has been
                    cancelled.
                  </p>
                </div>
              )}

              {errorsLoanApp.status && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm font-semibold text-gray-900">
                    {watchLoanApp("application_date") &&
                      dayjs(watchLoanApp("application_date")).format(
                        "MMM D, YYYY"
                      )}
                  </div>
                  <input
                    type="hidden"
                    {...registerLoanApp("application_date")}
                  />
                </div>
            ...
            <div className="bg-white p-3 rounded-lg border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm font-semibold text-gray-900 px-3 py-2 bg-gray-50 rounded border border-gray-200">
                    {watchLoanApp("loan_term")
                      ? `${watchLoanApp("loan_term")} months`
                      : "N/A"}
                  </div>
                  {errorsLoanApp.loan_term && (
                    <p className="text-error text-xs mt-1">Required</p>
                  )}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="px-4 py-1 bg-blue-50 rounded-lg border-2 border-blue-200">
                    <div className="text-lg font-bold text-blue-900">
                      ₱
                      {watchLoanApp("amount")
                        ? parseFloat(watchLoanApp("amount")).toLocaleString(
                            "en-US",
                            {
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  {...registerLoanApp("purpose", { required: false })}
                  className="hidden"
                />
                {errorsLoanApp.purpose && (
                  <p className="text-error text-xs mt-1">Required</p>
                )}
              </div>
            </div>
            ...
            <div className="p-3 bg-green-50 rounded-lg border-2 border-green-300 mb-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="md:col-span-1">
                  <label className="block text-xs font-medium text-gray-500 mb-1">
                    Club&apos;s Share Capital Balance
                  </label>
                  <div
                    className={`px-4 py-2 rounded-lg border-2 flex items-center justify-between gap-3 ${
                      isErrorShareCapital
                  ...
                  {isErrorShareCapital && errorShareCapital && (
                    <p className="text-xs text-red-500 mt-1 truncate">
                      {(
                        errorShareCapital?.message || "Unknown error"
                      ).toString()}
                    </p>
                  )}
                </div>
                  ...
                  rules={{
                    ...
                    validate: (value) => {
                      ...
                      try {
                        ...
                        const D_min = new Decimal(
                          selectedProduct?.min_amount || 0
                        );
                        const D_max =
                          productCode === "S_CAP_LOANS"
                            ...
                            : new Decimal(
                                selectedProduct?.max_amount || 9999999
                              );

                        if (D_value.lt(D_min))
                          return `Amount must be at least ₱${D_min.toFixed(2)}`;
                        if (productCode === "S_CAP_LOANS") {
                          const shareCap = new Decimal(
                          ...
                          return D_value.gt(D_max)
                            ? `Amount must not exceed ₱${D_max.toFixed(2)}`
                            : true;
                        }
                        return true;
                      } catch {
            ...
            <div className="bg-white p-3 rounded-lg border border-gray-200">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 rounded-full border border-purple-200">
                  ...
                  <span className="text-xs font-bold">
                    {watchLoanAcc("loan_term_approved")} months
                  </span>
                  <input
                    type="hidden"
                    {...registerLoanAcc("loan_term_approved", {
                      required: true,
                    })}
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <div
                    className={`relative px-4 py-2 bg-gradient-to-r from-purple-50 to-violet-50 rounded-lg border-2 border-purple-300 ${isCalculating ? "opacity-50" : ""}`}
                  >
                    <div className="text-xl font-bold text-purple-900 text-right">
                      <span
                        title={`₱${display(watchLoanAcc("service_fee")) || "0.00"}`}
                  ...
                  <input
                    type="hidden"
                    {...registerLoanAcc("service_fee", { required: true })}
                  />
                </div>
                ...
                <div>
                  ...
                  <div
                    className={`relative px-4 py-2 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg border-2 border-blue-300 ${isCalculating ? "opacity-50" : ""}`}
                  >
                    <div className="text-xl font-bold text-blue-900 text-right">
                      <span
                        title={`₱${display(watchLoanAcc("total_interest")) || "0.00"}`}
                        className="truncate block max-w-full"
                      >
                  ...
                  <input
                    type="hidden"
                    {...registerLoanAcc("total_interest", { required: true })}
                  />
                </div>
                ...
                <div>
                  ...
                  <div
                    className={`relative px-4 py-2 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border-2 border-emerald-300 ${isCalculating ? "opacity-50" : ""}`}
                  >
                    <div className="text-xl font-bold text-emerald-900 text-right">
                      <span
                        title={`₱${display(watchLoanAcc("monthly_payment")) || "0.00"}`}
                        className="truncate block max-w-full"
                      >
                  ...
                  <input
                    type="hidden"
                    {...registerLoanAcc("monthly_payment", { required: true })}
                  />
                </div>
                ...
                <div>
                  ...
                  <div
                    className={`relative px-4 py-2 bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg border-2 border-amber-300 ${isCalculating ? "opacity-50" : ""}`}
                  >
                    <div className="text-xl font-bold text-right">
                      <span
                        title={`₱${watchLoanAcc("total_amount_due") ? parseFloat(watchLoanAcc("total_amount_due")).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0.00"}`}
                        className="truncate block max-w-full"
                      >
                        ₱
                        {watchLoanAcc("total_amount_due")
                          ? parseFloat(
                              watchLoanAcc("total_amount_due")
