 ...
 */
function LoanScheduleCardList({ data = [], isLoading }) {
  return (
    <section className="border border-base-content/5 bg-white rounded-2xl shadow-md flex flex-col">
      <div className="p-4 max-h-[81vh] min-h-[17vh] overflow-y-auto space-y-3 bg-white rounded-2xl">
        {isLoading ? (
          ...
          data.map((item) => {
            const isOverdue =
              !item.paid && dayjs(item.due_date).isBefore(dayjs(), "day");
                    ...
                    >
                      <div className="text-center">
                        <div className="text-xs font-semibold opacity-60">
                          {dayjs(item.due_date).format("MMM")}
                        </div>
...
LoanScheduleCardList.propTypes = {
  data: propsTypes.arrayOf(
    propsTypes.shape({
      schedule_id: propsTypes.oneOfType([propsTypes.string, propsTypes.number])
        .isRequired,
