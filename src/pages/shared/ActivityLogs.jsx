  ...
  const logsData = useMemo(() => {
    ...
    return logsRaw.filter((row) => {
      ...
      const matchesSearch =
        ...
        (row.action &&
          row.action.toLowerCase().includes(debouncedSearch.toLowerCase())) ||
      ...
      const matchesTimeRange =
        cutoffTime === null ||
        (row.timestamp && Date.parse(row.timestamp) >= cutoffTime);
          ...
          renderRow={(row) => {
            ...
            return (
              <tr key={id} className="hover:bg-base-200/50">
                {/* Ref No. */}
                <td className="text-center font-medium text-xs w-[12%]">
                  {TABLE_PREFIX}_{id}
                </td>
                ...
                <td className="text-center font-semibold w-[15%]">
                  <span
                    className={`badge badge-soft ${ACTIVITY_LOGS_TYPE_COLORS[type] || "badge-error"}`}
                  >
                    {type}
                  </span>
                </td>
                ...
                {/* Action */}
                <td className="text-left w-[45%]">{action}</td>
              </tr>
            );
          }}
        />
