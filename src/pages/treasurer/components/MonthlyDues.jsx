...
function MonthlyDues() {
  ...
  const {
    data: monthlyDuesRecords,
    isLoading,
    isError,
    error,
  } = useFetchMonthlyDuesRecords({
    year: selectedYear,
  ...
  const selectedMemberName = useMemo(() => {
    ...
    const member = memberOptions.find(
      (m) => m.account_number === selectedMember
    );
    return member
    ...
    () => [
      {
        ...
        header: "Members",
        cell: (info) => (
          <div className="font-sm text-base-content whitespace-nowrap">
            {info.getValue()}
          </div>
        ),
        meta: { isSticky: true },
      ...
      ...filteredMonths.map((month) => ({
        ...
        cell: (info) => {
          ...
          return (
            <div
              className={`px-3 py-2 rounded-md font-semibold text-center ${
                value === "Paid"
        ...
        <div className="bg-base-100 rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-green-700 px-6 py-4">
            <h2 className="text-xl font-semibold text-white">
              Club Funds Monthly Payables Record ({selectedYear}) -{" "}
                      ...
                      {headerGroup.headers.map((header, index) => (
                        <th
                          ...
                          className={`px-4 py-3 text-center text-sm font-semibold text-base-content uppercase tracking-wider ${
                            index === 0
                              ? "sticky left-0 bg-base-200/80 backdrop-blur-md z-[25]"
                              : ""
                          }`}
                        >
                          {flexRender(
